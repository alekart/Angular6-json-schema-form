import {isArray, isEmpty, isEqual, isNumber, isObject, isString, has} from 'lodash';
import {commonItems, uniqueItems} from './utility.functions';
...
export function mergeSchemas(...schemas) {
  ...
  for (const schema of schemas) {
    for (const key of Object.keys(schema)) {
      ...
      if (!has(combinedSchema, key) || isEqual(combinedValue, schemaValue)) {
        ...
        switch (key) {
          ...
          case 'not':
            ...
            if (isObject(combinedValue) && isObject(schemaValue)) {
              const notAnyOf = [combinedValue, schemaValue]
                .reduce((notAnyOfArray: [], notSchema: any) =>
          ...
          case 'properties':
            ...
            if (isObject(combinedValue) && isObject(schemaValue)) {
              ...
              if (has(schemaValue, 'additionalProperties')) {
              ...
              for (const subKey of Object.keys(schemaValue)) {
                if (isEqual(combinedObject[subKey], schemaValue[subKey]) || (
                  !has(combinedObject, subKey) &&
                  !has(combinedObject, 'additionalProperties')
                )) {
                  combinedObject[subKey] = schemaValue[subKey];
